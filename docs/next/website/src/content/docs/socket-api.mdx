...
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.explain`, `agent.send_keys`, `agent.prompt`, `agent.prompt_if_idle`, `agent.wait`, `agent.rename`, `agent.focus`, `agent.start`, `agent.view.set`, `agent.view.clear` |
...
Use `agent.prompt_if_idle` when the caller must not submit into a turn that
started after it inspected the agent. Pass the `terminal_id` and
`state_change_seq` from `agent.get`; Herdr validates those values and the idle
state in the same app event-loop turn that queues the prompt and delayed Enter:

```json
{"id":"req_guarded_prompt","method":"agent.prompt_if_idle","params":{"target":"reviewer","text":"Review the current diff","expected_terminal_id":"term_123","expected_state_change_seq":42}}
```

It returns `agent_changed` when the pane occupant differs,
`agent_state_changed` when the lifecycle sequence moved, `agent_not_idle` for
a working agent, `agent_blocked` for a blocker, and `agent_not_ready` when the
state is unknown. Every rejection occurs before terminal input is queued. This
method intentionally has no `wait` option; use `agent.prompt` when prompting an
already-working agent is deliberate.
